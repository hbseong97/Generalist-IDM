...
async def import_file(mcap_file: UploadFile, mkv_file: UploadFile) -> OWAFile:
    ...
    if settings.PUBLIC_HOSTING_MODE:
        ...
        if mcap_file.size > size_limit:
            raise HTTPException(
                status_code=400,
                detail="MCAP File size exceeds 100MB limit. Please self-host the viewer for files larger than 100MB. For more info, see https://anonymous.github.io/anonymous/data/viewer.",
            )
        if mkv_file.size > size_limit:
            raise HTTPException(
                status_code=400,
                detail="MKV File size exceeds 100MB limit. Please self-host the viewer for files larger than 100MB. For more info, see https://anonymous.github.io/anonymous/data/viewer.",
            )
